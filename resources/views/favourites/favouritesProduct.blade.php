        ...
        await database.collection('vendor_products').where("id", "==", productID).get().then(async function (snapshotss) {
            ...
            if (snapshotss.docs[0]) {
              ...
              if (product.hasOwnProperty('disPrice') && product.disPrice != '' && product.disPrice != '0') {
                  product.disPrice = parseFloat(product.disPrice);
                  if (currencyAtRight) {
                    var dis_price = product.disPrice.toFixed(decimal_degits) + "" + currentCurrency;

                  } else {
                    var dis_price = currentCurrency + "" + product.disPrice.toFixed(decimal_degits);
                  }
