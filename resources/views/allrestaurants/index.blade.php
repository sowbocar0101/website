...
<a class="nav-link active border-0 bg-light text-dark rounded" id="home-tab" data-toggle="tab" href="#home" role="tab" aria-controls="home" aria-selected="true"><i class="feather-home mr-2"></i><span class="restaurant_counts">All Restaurants</span></a>
...
var placeholderImageRef = database.collection('settings').doc('placeHolderImage');
 ...
 async function callRestaurant() {
  ...
  async function getNearestRestaurants(){
    ...
    if(RestaurantNearBy){
        nearestRestauantRefnew=geoFirestore.collection('vendors').near({ center: new firebase.firestore.GeoPoint(address_lat, address_lng), radius: RestaurantNearBy});
    }else{
        nearestRestauantRefnew=geoFirestore.collection('vendors');
    }
...
sortArrayOfObjects = (arr, key) => {
 ...
 function buildHTMLNearestRestaurant(nearestRestauantSnapshot){
        ...
        nearestRestauantSnapshot.docs.forEach((listval) => {
            ...
            $('.restaurant_counts').text('{{trans('lang.popular_fashion_store')}}');
        }
        var count = 0;
